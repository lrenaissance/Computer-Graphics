/*
* <코드 작성. 컴파일 방법>
* HW1의 LearnOpenGL src 폴더에 9.robot_arm을 추가하였고,
* 그 안에 1.RobotArm이라는 폴더를 추가하여, 이 폴더에 HW3로 제공받은 파일들을 넣었습니다.
* LearnOpenGL의 CMakeLists.txt의
* set(CHAPTERS
	1.getting_started
	2.lighting
	3.model_loading
	...
	이렇게 두 개를 추가한 후, HW1처럼 Cmake GUI를 이용해 빌드한 LearnOpenGL sln 파일에서 수정하였습니다.
	wood가 나타나지 않아서, bin/9.robot_arm/Debug 파일에 wood.png를 넣어주었습니다.
	제가 HW2에서 만든 duck을 render하기 위해 bin/9.robot_arm/Debug 폴더에 duck.obj, duck.mtl, duck.jpg를 넣어주었습니다.
*/
...
#include <learnopengl/camera.h> //camera 관련
...
int RightButtonDown = 0; //right mouse
int RobotControl = 0;
...
bool secondMouse = true; //right mouse
...
const char* ourObjectPath = "./duck.obj"; //teapot.obj을 duck.obj로 수정
// translate it so it's at the center of the scene
// it's a bit too big for our scene, so scale it down

glm::mat4 obj_model = glm::rotate(glm::mat4(1.0f), glm::radians(-110.0f), glm::vec3(0.0f, 1.0f, 0.0f));//오리가 정면을 보도록 회전

glm::mat4 objectXform = glm::scale(glm::translate(obj_model, glm::vec3(-0.3f, 0.1f, -0.3f)), glm::vec3(0.014f, 0.014f, 0.014f)); //오리 배치하고 0.014배로 크기 조정
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset); //for mouse scroll
...
void myDisplay()
{
	...
	DrawWrist(model);
	glm::mat4 finger_model = model;//for right finger
	...
	//finger base(right)
	finger_model = glm::translate(finger_model, glm::vec3(0.0f, 0.2f, 0.0f));
	finger_model = glm::rotate(finger_model, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerBase(finger_model);

	//finger tip(right)     
	finger_model = glm::translate(finger_model, glm::vec3(0.0f, 0.35f, 0.0f));
	finger_model = glm::rotate(finger_model, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawFingerTip(finger_model);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		PhongShader->use();
		PhongShader->setMat4("projection", glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f)); //proj 설정
		PhongShader->setMat4("view", camera.GetViewMatrix()); //view 불러오기
...
void initGL(GLFWwindow** window)
{
	...
	* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "The Robot Arm", NULL, NULL);
	...
	glfwSetScrollCallback(*window, scroll_callback); //scroll 추가
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//wasd key(너무 느려서 deltaTime의 100배로 설정)
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	//right mouse(change view direction)
	if (RightButtonDown) {
		//view direction 바뀌는 속도가 너무 느려서 기존 offset의 100배로 설정
