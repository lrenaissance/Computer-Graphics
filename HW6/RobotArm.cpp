...
* set(CHAPTERS
	...
	teapot이랑 wood가 나타나지 않아서, bin/9.robot_arm/Debug 파일에 teapot.obj와 wood.png를 넣어주었습니다.
*/
...
//toggle light(added)
int LKeyPressed = -1; //true(1): spotlight on, false(-1): spotlight off
bool LKey = false;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//added for angular attenuation
		PhongShader->setBool("LKey", LKey);
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS && LKeyPressed==-1) {//l: toggle angular attenuation(added)
		LKey=!LKey;
		LKeyPressed=LKeyPressed*(-1);
	}
